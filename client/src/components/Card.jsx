...
const Card = ({ image, title, category, profile, amount, button, slug }) => {
  return (
    <div className="flex flex-col w-80 shadow-xl mx-3 gap-2 justify-center bg-gray-100 min-h-[25rem] bg-opacity-40 hover:scale-105 transition-all duration-300 items-center h-54 hover:shadow-[#393948] hover:shadow-lg m-8 rounded-3xl">
      {!button && (
        <div className="relative h-[90%] max-h-[15rem] w-[90%] mt-4 rounded-3xl overflow-hidden">
          <img
            src={image}
            alt="placeholder"
            className="h-full w-full object-cover rounded-3xl min-h-[15rem]"
          />
